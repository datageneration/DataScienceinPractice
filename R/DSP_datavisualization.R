...
install.packages(c("haven","descr","tidyverse", "png","gifski", "gapminder","gganimate","RColorBrewer","Rcpp"),repos = "http://cran.rstudio.com")
lapply(c("haven","descr","tidyverse", "png","gifski", "gapminder","gganimate","RColorBrewer","Rcpp"), require, character.only = TRUE)
...
ggplot(gapminder, mapping = mapping) + # Gather mapping data
  ...
  # Labels
  geom_text(aes(label=ifelse((country == "China"), "China", ifelse(country=="United States", "United States", ifelse(country=="Taiwan", "Taiwan","")))),vjust=0,nudge_y = 1,size=6) +
  # Add text labels
  transition_time(year) + # Control animation by year
  ease_aes('linear') # Control easing of aesthetics (constant speed)

## Exercise: Can you label Taiwan and South Korea (Korea Rep) in the animated chart? Make sure you don't mix up with North Korea.
